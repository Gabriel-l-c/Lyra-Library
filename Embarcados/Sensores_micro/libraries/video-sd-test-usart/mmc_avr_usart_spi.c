...
/ 
/  Modified by Nicola Wrachien (next-hack.com) in 2017
/
...
#include "display.h"    // added by next-hack.com
#include <avr/io.h>
#include "diskio.h"
#include "mmc_avr.h"
/* Peripheral controls (Platform dependent) */
#define CS_LOW()		PORTD &=  ~(1<<2) 	/* Set MMC_CS = low */
#define	CS_HIGH()		PORTD |= (1<<2)	/* Set MMC_CS = high */
...
#define	FCLK_SLOW()		UBRR0 = 31	/* Set SPI slow clock (100-400kHz) - Set to 250kHz @ fCPU=16M */
#define	FCLK_FAST()		UBRR0 = 0	/* Set SPI fast clock (20MHz max) */
...
static int datablock_to_display_direct (  UINT btr);   // added by next-hack.com
static void spi_multi_mmc_to_display_transfer ();     // added by next-hack.com
...
DSTATUS mmc_disk_initialize (void)
{
  ...
  #warning next-hack removed these two lines!
...
/*
 *  new functions added by next-hack.com for direct SD to Display transfer
 */

DRESULT mmc_to_display_direct_transfer (
  DWORD sector,   /* Start sector number (LBA) */
  UINT count      /* Sector count (1..128) */
)
{ 
...
//  if (!(CardType & CT_BLOCK)) sector *= 512;  /* Convert to byte address if needed */
  ...
  if (send_cmd(cmd, sector) == 0) {
    do {
      if (!datablock_to_display_direct(512)) break;
          } while (--count);
    if (cmd == CMD18) send_cmd(CMD12, 0); /* STOP_TRANSMISSION */
  }
  deselect();
...
static int datablock_to_display_direct (
  UINT btr      /* Byte count (must be multiple of 4) */
)
{
  BYTE token;
  ...
  spi_multi_mmc_to_display_transfer();    /* Receive the data block into buffer */
  xchg_spi(0xFF);         /* Discard CRC */
  xchg_spi(0xFF);
...
static void spi_multi_mmc_to_display_transfer ()
{
  volatile char dummy;
  SelectDisplay();
   int cnt = 511;
